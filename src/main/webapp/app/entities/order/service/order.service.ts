...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'collectionDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      collectionDate: order.collectionDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      collectionDate: restOrder.collectionDate ? dayjs(restOrder.collectionDate) : undefined,
    };
  }
